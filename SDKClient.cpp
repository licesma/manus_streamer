...
ClientReturnCode SDKClient::Initialize()
{
	...
	// Skeleton publisher on port 8000
	sock_skel = zmq::socket_t(ctx, zmq::socket_type::push);
	sock_skel.bind("tcp://*:8000");

	// Ergonomics publisher on port 8001
	sock_ergo = zmq::socket_t(ctx, zmq::socket_type::push);
	sock_ergo.bind("tcp://*:8001");

	// Per-hand skeleton publishers
	sock_left_skel = zmq::socket_t(ctx, zmq::socket_type::push);
	sock_left_skel.bind("tcp://*:8002");

	sock_right_skel = zmq::socket_t(ctx, zmq::socket_type::push);
	sock_right_skel.bind("tcp://*:8003");
	...
	if (s_Instance)
	{
		...
		try {
			...
			for (unsigned int j = 0; j < p_RawSkeletonStreamInfo->skeletonsCount; j++) {
				// Build per-hand buffer for the separate sockets
				char hand_buf[4096] = { 0 };
				char* hpos = hand_buf;
				int hleng = 0;
				...
				if (full) {
					for (unsigned int i = 0; i < int(s_Instance->m_NextRawSkeleton->skeletons[0].info.nodesCount); i++) {
						...
						int ht = sprintf(hpos, "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,", position_glove.x, position_glove.y, position_glove.z, rotation_glove.x, rotation_glove.y, rotation_glove.z, rotation_glove.w);
						hpos = hpos + ht;
						hleng = hleng + ht;
					}
				}
					...
					for (unsigned int i = 0; i < 10; i++) {
						...
						int ht = sprintf(hpos, "%.3f,%.3f,%.3f,", position_glove.x, position_glove.y, position_glove.z);
						hpos = hpos + ht;
						hleng = hleng + ht;
					}
				}

				// Send to per-hand socket (no gloveId prefix, just the data)
				if (hleng > 0) {
					hleng -= 1; // remove trailing comma
					uint32_t gid = s_Instance->m_NextRawSkeleton->skeletons[j].info.gloveId;
					if (gid == s_Instance->m_FirstLeftGloveID) {
						s_Instance->sock_left_skel.send(zmq::buffer(hand_buf, hleng), zmq::send_flags::dontwait);
					} else if (gid == s_Instance->m_FirstRightGloveID) {
						s_Instance->sock_right_skel.send(zmq::buffer(hand_buf, hleng), zmq::send_flags::dontwait);
					}
				}
			}
			leng3 += -1;

			// Legacy combined socket (kept for backwards compatibility)
			s_Instance->sock_skel.send(zmq::buffer(char_buf_3, leng3), zmq::send_flags::dontwait);
